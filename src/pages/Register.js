...
const Register = () => {
    ...
    const handleEmailVerification = async () => {
      try {
        const response = await fetch(`http://localhost:6639/request-email-verification/${email}`, {
          method: 'GET',
    ...
    const handleSubmit = async (e) => {
      ...
      const response = await fetch('http://localhost:6639/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <Container>
      <Body dir="column">
        ...
        <div>
          <TextBox placeholder="email" onChange={setEmail} />
